...
export function registerNodeBalancerTools(server: FastMCP, client: LinodeClient) {
  ...
  server.addTool({
    ...
    execute: withErrorHandling(async (params: any, context?: any) => {
      const result = await createClient(context).nodeBalancers.getNodeBalancer(params.id);
      return JSON.stringify(result, null, 2);
    })
  });
  server.addTool({
  ...
  server.addTool({
    ...
    execute: withErrorHandling(async (params: any, context?: any) => {
      const result = await createClient(context).nodeBalancers.getNodeBalancerConfigs(params.nodebalancer_id);
      return JSON.stringify(result, null, 2);
    })
  });
  server.addTool({
  ...
  server.addTool({
    ...
    execute: withErrorHandling(async (params: any, context?: any) => {
      const result = await createClient(context).nodeBalancers.getNodeBalancerConfigNodes(params.nodebalancer_id, params.config_id);
      return JSON.stringify(result, null, 2);
    })
  });
  server.addTool({
