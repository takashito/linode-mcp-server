import { z } from "zod";
import { pagingParamsSchema, tagSchema, tagsSchema, paginationSchema, paginatedResponseSchema, coerceBoolean } from '../common/schemas';

// Config Interface schema (moved to the top for dependency resolution)
export const configInterfaceSchema = z.object({
  id: z.coerce.number().describe('The ID of the interface'),
  ...
  primary: coerceBoolean().optional().describe('Whether this is the primary interface'),
  active: coerceBoolean().optional().describe('Whether the interface is active'),
  subnet_id: z.coerce.number().optional().describe('The subnet ID for VPC interfaces'),
  vpc_id: z.coerce.number().optional().describe('The VPC ID for VPC interfaces'),
  ipv4: z.object({
...
const linodeSpecsSchema = z.object({
  disk: z.coerce.number().describe('The amount of storage space in MB'),
  memory: z.coerce.number().describe('The amount of RAM in MB'),
  vcpus: z.coerce.number().describe('The number of virtual CPUs'),
  transfer: z.coerce.number().describe('The amount of network transfer available in GB')
});

const linodeAlertsSchema = z.object({
  cpu: z.coerce.number().describe('CPU usage threshold percentage (0-400)'),
  io: z.coerce.number().describe('Disk IO threshold in ops/sec'),
  network_in: z.coerce.number().describe('Incoming network traffic threshold in Mbps'),
  network_out: z.coerce.number().describe('Outgoing network traffic threshold in Mbps'),
  transfer_quota: z.coerce.number().describe('Network transfer quota threshold in GB')
});

const linodeBackupsSchema = z.object({
  enabled: coerceBoolean().describe('Whether backups are enabled'),
  schedule: z.object({
...
export const linodeInstanceSchema = z.object({
  id: z.coerce.number().describe('The unique identifier for this Linode'),
  ...
  watchdog_enabled: coerceBoolean().describe('Whether the watchdog service is enabled')
});

export const linodeConfigSchema = z.object({
  id: z.coerce.number().describe('The config ID'),
  ...
  memory_limit: z.coerce.number().describe('The memory limit in MB for this configuration'),
  ...
  helpers: z.object({
    updatedb_disabled: coerceBoolean().describe('Whether updatedb is disabled in this configuration'),
    distro: coerceBoolean().describe('Whether the distro helper is enabled'),
    network: coerceBoolean().describe('Whether the network helper is enabled'),
    modules_dep: coerceBoolean().describe('Whether the modules dependency helper is enabled')
  }),
...
export const linodeDiskSchema = z.object({
  id: z.coerce.number().describe('The disk ID'),
  label: z.string().describe('The disk\'s label'),
  status: z.string().describe('The disk\'s status'),
  size: z.coerce.number().describe('The disk size in MB'),
...
export const kernelSchema = z.object({
  ...
  kvm: coerceBoolean().describe('Whether this kernel supports KVM virtualization'),
  architecture: z.string().describe('The architecture this kernel supports'),
  pvops: coerceBoolean().describe('Whether this kernel supports paravirtualized operations'),
  deprecated: coerceBoolean().describe('Whether this kernel is deprecated'),
...
export const getInstanceSchema = z.object({
  id: z.coerce.number().describe('The ID of the Linode instance')
});
...
export const createInstanceSchema = z.object({
  ...
  backups_enabled: coerceBoolean().optional().describe('Whether backups are enabled'),
  booted: coerceBoolean().optional().describe('Whether the Linode should be booted after creation'),
  private_ip: coerceBoolean().optional().describe('Whether to assign a private IP'),
  tags: tagSchema,
  firewall_id: z.coerce.number().optional().describe('ID of the firewall to assign this Linode to')
});
...
export const updateInstanceSchema = z.object({
  ...
  group: z.string().optional().describe('The group for the Linode'),
  tags: tagSchema,
  watchdog_enabled: coerceBoolean().optional().describe('Whether the watchdog service is enabled'),
  alerts: z.object({
    cpu: z.coerce.number().optional().describe('CPU usage threshold percentage (0-400)'),
    io: z.coerce.number().optional().describe('Disk IO threshold in ops/sec'),
    network_in: z.coerce.number().optional().describe('Incoming network traffic threshold in Mbps'),
    network_out: z.coerce.number().optional().describe('Outgoing network traffic threshold in Mbps'),
    transfer_quota: z.coerce.number().optional().describe('Network transfer quota threshold in GB')
  }).optional()
});

export const rebootInstanceSchema = z.object({
  id: z.coerce.number().describe('The ID of the Linode instance'),
  config_id: z.coerce.number().optional().describe('The ID of the configuration profile to boot into')
});
...
export const resizeInstanceSchema = z.object({
  ...
  allow_auto_disk_resize: coerceBoolean().optional().describe('Whether to automatically resize disks (default: true)')
});

export const cloneInstanceSchema = z.object({
  id: z.coerce.number().describe('The ID of the Linode instance to clone'),
  ...
  backups_enabled: coerceBoolean().optional().describe('Whether backups are enabled for the new Linode'),
  private_ip: coerceBoolean().optional().describe('Whether to assign a private IP to the new Linode'),
  tags: tagSchema
});
...
export const rebuildInstanceSchema = z.object({
  ...
  authorized_keys: z.array(z.string()).optional().describe('SSH public keys to deploy to the root user (recommended for better security)'),
  authorized_users: z.array(z.string()).optional().describe('Linode usernames who can deploy their SSH keys to this Linode'),
  stackscript_id: z.coerce.number().optional().describe('StackScript ID to use for deployment. Run List StackScripts to get available IDs.'),
  ...
  booted: coerceBoolean().optional().describe('Whether the Linode should be booted after rebuild')
});

export const rescueInstanceSchema = z.object({
  id: z.coerce.number().describe('The ID of the Linode instance'),
  devices: z.record(z.coerce.number()).describe('Block device assignments for /dev/sdX devices')
});

// Config operations
export const getLinodeConfigsSchema = z.object({
  linodeId: z.coerce.number().describe('The ID of the Linode instance'),
  ...pagingParamsSchema.shape
});

export const getLinodeConfigSchema = z.object({
  linodeId: z.coerce.number().describe('The ID of the Linode instance'),
  configId: z.coerce.number().describe('The ID of the configuration profile')
});
...
export const createLinodeConfigSchema = z.object({
  ...
  memory_limit: z.coerce.number().optional().describe('The memory limit in MB'),
  ...
  helpers: z.object({
    updatedb_disabled: coerceBoolean().optional().describe('Whether updatedb is disabled'),
    distro: coerceBoolean().optional().describe('Whether the distro helper is enabled'),
    network: coerceBoolean().optional().describe('Whether the network helper is enabled'),
    modules_dep: coerceBoolean().optional().describe('Whether the modules dependency helper is enabled')
  }).optional(),
...
export const updateLinodeDiskSchema = z.object({
  ...
  size: z.coerce.number().optional().describe('The new size of the disk in MB')
});
...
export const resizeLinodeDiskSchema = z.object({
  linodeId: z.coerce.number().describe('The ID of the Linode instance'),
  diskId: z.coerce.number().describe('The ID of the disk'),
  size: z.coerce.number().describe('The new size of the disk in MB')
});
...
export const backupSchema = z.object({
  id: z.coerce.number().describe('The ID of the backup'),
  ...
  available: coerceBoolean().describe('Whether the backup is available for restore')
});
...
export const getBackupSchema = z.object({
  linodeId: z.coerce.number().describe('The ID of the Linode instance'),
  backupId: z.coerce.number().describe('The ID of the backup')
});
...
export const restoreBackupSchema = z.object({
  linodeId: z.coerce.number().describe('The ID of the Linode instance'),
  backupId: z.coerce.number().describe('The ID of the backup'),
  linode_id: z.coerce.number().optional().describe('Target Linode ID to restore to (defaults to the source Linode)'),
  overwrite: coerceBoolean().optional().describe('Whether to overwrite the target Linode')
});
...
export const linodeAllocateIPSchema = z.object({
  ...
  public: coerceBoolean().describe('Whether the IP address should be public')
});
...
export const updateLinodeFirewallsSchema = z.object({
  linodeId: z.coerce.number().describe('The ID of the Linode instance'),
  ids: z.array(z.coerce.number()).describe('Array of firewall IDs to assign to this Linode')
});
...
export const reorderConfigInterfacesSchema = z.object({
  linodeId: z.coerce.number().describe('The ID of the Linode instance'),
  configId: z.coerce.number().describe('The ID of the configuration profile'),
  ids: z.array(z.coerce.number()).describe('The ordered list of interface IDs')
});
